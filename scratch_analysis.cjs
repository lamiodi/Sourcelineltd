...
const points = lines.map((l, idx) => {
  ...
  return { id: name, code, easting: parseFloat(easting), northing: parseFloat(northing), elevation: parseFloat(elevation), rawIndex: idx };
});
...
for (const [code, pts] of Object.entries(groups)) {
  ...
  console.log(`${code} (${pts.length} pts): Easting [${minE.toFixed(3)} to ${maxE.toFixed(3)}], Northing [${minN.toFixed(3)} to ${maxN.toFixed(3)}]`);
}
...
for (let i = 0; i < points.length; i++) {
  for (let j = i + 1; j < points.length; j++) {
    ...
    if (d < 0.005) {
      console.log(`Duplicate found: ${points[i].id} and ${points[j].id} at E: ${points[i].easting}, N: ${points[i].northing}`);
      dupFound = true;
    }
  }
}
if (!dupFound) console.log('No duplicates.');

console.log('\n--- Detailed Group Analysis ---');
for (const [code, pts] of Object.entries(groups)) {
  console.log(`\n=== Group: ${code} (${pts.length} points) ===`);
  for (let i = 0; i < pts.length; i++) {
    if (i < pts.length - 1) {
      ...
      console.log(`  ${p1.id} -> ${p2.id}: Dist = ${dist.toFixed(3)}m, Bearing = ${brg.toFixed(1)}°`);
    }
  }
}
